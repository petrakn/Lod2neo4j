...
class BaseNeo4jConnector(object):
    ...
    def call(self, data, method='post', url=None, reverse=1):
        ...
        result = getattr(requests, method)(url, data=datastring, headers=self.headers)  # requests.post(url, data=datastring, headers=self.headers)

        if self.timed:
            print 'time: ', time.time() - start

        if self.transactionurl is None:
    ...
    def query(self, statements):
        if type(statements) == str or type(statements) == unicode:
